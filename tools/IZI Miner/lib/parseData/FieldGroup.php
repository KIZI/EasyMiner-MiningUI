...
class FieldGroup {
    ...
    function  __construct(DOMElement $FGNode, DOMXPath $XPath, $lang, $isRoot, &$attributes, &$coefficients) {
        $this->FGNode = $FGNode;
        $this->XPath = $XPath;
        $this->lang = $lang;
        $this->isRoot = $isRoot;
    ...
    protected function parseFieldConfig($fcNode) {
        ...
        $minimalLength = intval($this->XPath->evaluate('Coefficient/MinimalLength', $fcNode)->item(0)->nodeValue);
        $maximalLength = intval($this->XPath->evaluate('Coefficient/MinimalLength', $fcNode)->item(0)->nodeValue);
        $category = $this->XPath->evaluate('Coefficient/Category', $fcNode)->item(0)->nodeValue;
