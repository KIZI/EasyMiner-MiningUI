...
class FieldGroupConfigParser {
    ...
    function __construct(DOMDocument $FGC, &$attributes, &$coefficients, $lang) {
        $this->FGC = $FGC;
    ...
    public function parseConfig() {
        ...
        // disabled
        // $this->initMissingAttributes($config);
    ...
    protected function parseRootConfigID() {
        $array = array();
        $this->rootConfigID = intval($this->XPath->evaluate('/fg:FieldGroupConfigs/@rootConfigID')->item(0)->value);
        if ($this->rootConfigID <= 0) {
