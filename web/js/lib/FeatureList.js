var FeatureList = new Class({
  ...
  /**
   * Funkce vracející pole měr zajímavosti pro vykreslení v rámci nalezených pravidel
   * @param ruleIMNames
   * @returns {Array}
   */
  getRulesIMs: function (ruleIMNames){
    var defaultIMs=this.getDefaultIMs();
    var IMs=[];
    var usedIMNames=ruleIMNames;
    if(!Array.isArray(usedIMNames)){
      usedIMNames=[];
    }

    Array.each(defaultIMs,function(IM){
      var IMName=IM.getName();
      if (usedIMNames.indexOf(IMName)==-1){
        usedIMNames.push(IMName);
      }
    }.bind([this,usedIMNames]));

    Array.each(usedIMNames, function (IMName) {
      IMs.push(this.getIM(IMName));
    }.bind(this));
    ...
    switch (type) {
      case 'Conjunction':
        ...
        Object.each(settings, function (setting, lvl) {
          if (setting['Conjunction'] || setting['Disjunction']) {
            connectiveUsed++;
          }
        }.bind(this));
  ...
  hasCedentNoRestriction: function (cedent) {
    if (cedent.getNumFields() > 0) {
      return false;
    } // only empty cedents can have no restriction
